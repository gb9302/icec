...
const server=http.createServer(async(req,res)=>{try{
 if(req.url==='/api/health'){await pool.query('SELECT 1');return send(res,200,{ok:true,version:'0.3.2'})}
 if(req.url==='/api/integrity'&&req.method==='GET'){return send(res,200,await integrityReport())}
 if(req.url==='/api/export'&&req.method==='GET'){const x=await loadState();return send(res,200,{format:'icec-lab-export',version:'0.3.2',exportedAt:new Date().toISOString(),...x},{'content-disposition':'attachment; filename=icec-lab-export.json'})}
...
server.listen(3001,'0.0.0.0',()=>console.log('IceC API v0.3.2 listening on 3001'));
